...
```
Usuario (Postman)
    ↓ POST /api/facturas/procesar (imagen/PDF)
...
MCP Server TypeScript + Express (Puerto 3001)
    ...
    ↓ Ejecuta tools (buscar, validar, crear)
    ↓ Llama al Backend REST
    
Backend NestJS (Puerto 3002)
    ↓ SQLite database
    ↓ CRUD productos y egresos
```

## ✨ Características Principales

- 🤖 **Análisis inteligente de facturas** con Gemini AI
- 🔧 **Model Context Protocol (MCP)** para comunicación entre servicios
- 📋 **JSON-RPC 2.0** para invocación de tools
- 🎯 **Function Calling** automático de Gemini
- 💾 **SQLite** para persistencia de datos
- 🔍 **Búsqueda inteligente** de productos
- ✅ **Validación de stock** automática
- 📊 **Registro de egresos** completo
...
El backend usa TypeORM con `synchronize: true`, por lo que las tablas se crean automáticamente.

Para cargar productos de prueba, puedes ejecutar el SQL en `apps/backend/seed.sql` después de iniciar el backend por primera vez.
